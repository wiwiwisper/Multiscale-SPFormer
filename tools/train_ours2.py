...
from ours2_spformer.dataset import build_dataloader, build_dataset
from ours2_spformer.evaluation import ScanNetEval
from ours2_spformer.model import OurSPFormer2
from ours2_spformer.utils import AverageMeter, get_root_logger


def get_args():
    parser = argparse.ArgumentParser('OurSPFormer2')
...
def eval(epoch, model, dataloader, cfg, logger, writer):
    ...
    with open(metrics_file_path, 'a') as metrics_file:
        ...
        metrics_file.write(f'Epoch: {epoch}  |  Time: {timestamp}\n')
        metrics_file.write(f'{"="*64}\n')
        eval_res = scannet_eval.evaluate(pred_insts, gt_insts, logger, metrics_file)
...
def main():
    ...
    model = OurSPFormer2(**cfg.model).cuda()
