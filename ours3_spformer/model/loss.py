...
class Criterion(nn.Module):
    ...
    def __init__(
        ...
        num_class=18,
        leaf_mask_weight=1.5,   # extra weight on mask loss for leaf instances
        leaf_class_id=1,        # 0=stem, 1=leaf (0-indexed)
    ):
        super().__init__()
        ...
        self.leaf_mask_weight = leaf_mask_weight
        self.leaf_class_id = leaf_class_id
    ...
    def get_layer_loss(self, layer, aux_outputs, insts):
        ...
        # mask loss (leaf-weighted per instance)
        ...
        for mask, score, inst, (idx_q, idx_gt) in zip(pred_masks, pred_scores, insts, indices):
            ...
            pred_mask = mask[idx_q]       # (K, N)
            tgt_mask = inst.gt_spmasks[idx_gt].float()  # (K, N)
            with torch.no_grad():
            ...
            # per-instance weights: leaf instances get leaf_mask_weight
            inst_w = torch.ones(pred_mask.shape[0], device=pred_mask.device)
            inst_w[inst.gt_labels[idx_gt] == self.leaf_class_id] = self.leaf_mask_weight

            # per-instance BCE: (K,)
            bce_per = F.binary_cross_entropy_with_logits(
                pred_mask, tgt_mask, reduction='none').mean(-1)
            mask_bce_loss += (bce_per * inst_w).sum() / inst_w.sum()

            # per-instance Dice: (K,)
            p = pred_mask.sigmoid()
            numerator = 2 * (p * tgt_mask).sum(-1)
            denominator = p.sum(-1) + tgt_mask.sum(-1)
            dice_per = 1 - (numerator + 1) / (denominator + 1)
            mask_dice_loss += (dice_per * inst_w).sum() / inst_w.sum()
