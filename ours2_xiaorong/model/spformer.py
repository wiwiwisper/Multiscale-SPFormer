...
class OurSPFormer2Ablation(nn.Module):
    """Ablation variant of OurSPFormer2.

    Supports:
      - fusion_mode: 'mlp' (cat + Linear + LN + ReLU) | 'add' (element-wise add)
      - coarse_scale_2: 0.0 = 2-scale mode; >0 = 3-scale mode (forces mlp fusion)
      - decoder.num_layer: ablate decoder depth via config
    """
    ...
    def __init__(
        ...
        num_class=18,
        geo_feat_dim: int = 0,
        coarse_scale: float = 0.5,    # first coarse-grid cell size (legacy)
        coarse_scale_2: float = 0.0,  # second coarse-grid cell size (legacy); 0 = disabled
        coarse_scales: list = None,   # N-scale list, overrides coarse_scale/coarse_scale_2
        fusion_mode: str = 'mlp',     # 'mlp' | 'add'
        decoder=None,
        criterion=None,
        test_cfg=None,
        norm_eval=False,
        ...
        # fusion layer (only used when fusion_mode == 'mlp')
    ...
    def train(self, mode=True):
        super(OurSPFormer2Ablation, self).train(mode)
    ...
    def create_coarse_sp(self, sp_coords, batch_offsets, scale):
        ...
        Args:
            ...
            scale:         float          coarse-grid cell size
        ...
        for b in range(B):
            ...
            grid = (coords_b / scale).long()                   # (M_fine_b, 3)
    ...
    def extract_feat(self, x, superpoints, v2p_map, coords_float, batch_offsets):
        ...
        # multi-scale coarse feature aggregation
        all_feats = [sp_feats]
        ...
        if self.num_streams > 2 or self.fusion_mode == 'mlp':
